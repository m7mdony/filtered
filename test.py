...
def check(images_path, labels_path):
    ...
    while current_index < len(image_list):
        ...
        if key == ord("q"):
            ...
            shutil.move(image_path, os.path.join(to_images_path, filename))
            
            # Move the label file to the target directory
            shutil.move(labels_path_file, os.path.join(t_labels_path, filename[:-4] + ".txt"))
            current_index+=1
        elif key == ord("d"):
            # Delete both the image and the label file
            os.remove(image_path)
            os.remove(labels_path_file)
            current_index += 1
        else:
